...
// Pie de página
    function Footer() {
        ...
        $this->SetTextColor(42, 46, 75);
        // Arial italic 12
        $this->MultiCell(200, 3, utf8_decode($leye), 0, 'C');
...
$array = ["id_orden"=>$id,"fecha_mod"=>$fechaActual];
$fechas = $cp->ejecutarSentencia(DBSentencias::VERIFICAR,array($id));
$verif=$fechas->fetchAll(PDO::FETCH_ASSOC);
...
if($val == '0'){
$fechaFinal = $cp->ejecutarSentencia(DBSentencias::FECHA_FINAL_ORDEN,$array);
$formatoFecha=date('d-m-Y',$fecha); 
}else{
$fechafinal=$cp->ejecutarSentencia(DBSentencias::CONSULTA_FINAL_ORDEN,array($id));
$final= $fechafinal->fetchAll(PDO::FETCH_ASSOC);
...
$tel = 0;
$leye = leye();
$leye1 = 'La empresa no se responsabiliza por el origen de los equipos recibidos. La empresa no se responsabiliza por la perdida de los equipos  por causa de siniestro, robo, hurto, o desastre natural. Todo software y/o información existente en medios de almacenamiento  son propiedad del cliente TCP/IP Tecnología no se responsabiliza por la información en medios de almacenamiento. La fecha de entrega del equipo puede variar de acuerdo a disponibilidad de repuestos, tiempos de reparación y prueba. Verifique la descripcion detallada de este documento caso contrario no habra lugar a reclamos, Conserve este documento ya que sin él no se podrá retirar el equipo. Pasado los 90 días el equipo se considerará abandonado y queda a disposición de TCP/IP Tecnología sin derecho a reclamo alguno. Luego de presupeustado el equipo, el cliente tiene 10 días para retirarlo caso contrario se actualizarán los costos.';
if ($_GET["estado"] < "13") {
    foreach ($ordenes as $orden) {
        if ($alterna) {
            ...
            $pdf->Ln();
            $mostrar= explode(":",$orden['falla_cliente_orden']);
            $pdf->multiCell(200, 3, utf8_decode(/*$orden['falla_cliente_orden']*/(string)$mostrar[0]), 0, 'L');
            $pdf->SetY(127);
            $pdf->SetFont('Arial', 'I', 6);
            $pdf->SetTextColor(42, 46, 75);
            
            // Arial italic 12
            $pdf->MultiCell(200, 3, utf8_decode($leye), 0, 'C');
            $pdf->SetTextColor(0, 120, 0);
            $pdf->SetFont('Arial', 'I', 5);
            $pdf->MultiCell(200, 3, utf8_decode($leye1), 0, 'C');
            $pdf->Ln();
            $pdf->SetTextColor(0, 0, 0);
            $pdf->multiCell(200, 1, utf8_decode("-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------"), 0, 0, 'C', true);
            $pdf->ln();
            $pdf->ln();
            $pdf->ln();
            $pdf->ln();
